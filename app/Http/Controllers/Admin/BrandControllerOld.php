                ...
                if($id!=""){
                    ...
                    foreach ($brandProducts as $key => $product) {
                        ...
                        if($product['discount_type']=="brand"){
                            Product::where('id',$product['id'])->update(['discount_type'=>'','product_discount'=>0,'final_price'=>$product['product_price']]);    
                        }
                    }
                }
            }else{
                // Add Brand Discount to all products belongs to the specific Brand
                Product::wherein('discount_type',['','brand'])->where('brand_id',$id)
                    ->update(array(
                        'discount_type' =>'brand',
                        'product_discount' => $data['brand_discount'],
                        'final_price' => DB::raw('product_price - (product_price * '.$data['brand_discount'].' / 100.0)')
                    ));    
            }
