            ...
            <tr class="cart-item">
                <td class="cart-item-media">
                    <div class="mini-img-wrapper">
                        <a
                            href="{{ route('product',[$cartitem['product']['product_url'],$cartitem['product']['id']]) }}">
                            @if(!empty($cartitem['product']['product_image']['image']))
                            <img class="mini-img"
                                src="{{ asset('front/images/products/small/'.$cartitem['product']['product_image']['image'])}}"" alt="
                                img">
                            @else
                            <img src="{{asset('front/images/no-image-found.jpg')}}"
                                alt="{{$cartitem['product']['product_name']}}" class="product-image">
                            @endif
                        </a>
                    </div>
                ...
                <td class="cart-item-details">
                    <h2 class="product-title"><a
                            href="{{ route('product',[$cartitem['product']['product_url'],$cartitem['product']['id']]) }}">{{
                            $cartitem['product']['product_name'] }}</a></h2>
                    <p class="product-vendor">Size: {{ $cartitem['size'] }}</p>
                </td>
                <td class="cart-item-quantity">
                    <div class="quantity d-flex align-items-center justify-content-between">
                        <button class="qty-btn dec-qty changeqty" data-size="{{ $cartitem['size'] }}"
                            data-proid="{{ $cartitem['product_id'] }}" data-cartid="{{ $cartitem['id'] }}"><img
                                src="{{ asset('front/assets/img/icon/minus.svg') }}" alt="minus"></button>
                        <input class="qty-input" type="number" id="qty-{{ $cartitem['id'] }}" name="qty"
                            value="{{ $cartitem['qty'] }}" min="0">
                        <button class="qty-btn inc-qty changeqty" data-size="{{ $cartitem['size'] }}"
                            data-proid="{{ $cartitem['product_id'] }}" data-cartid="{{ $cartitem['id'] }}"><img
                                src="{{ asset('front/assets/img/icon/plus.svg') }}" alt="plus"></button>
                    </div>
                    <a href="javascript:;" data-cartid="{{ $cartitem['id'] }}"
                        class="deleteCartItem product-remove mt-2">Remove</a>
                </td>
...
<div class="col-lg-4 col-md-12 col-12">
    <div class="cart-total-area">
        <div class="coupon field_form input-group mb-3">
            <input type="text" id="couponCode" name="coupon" @if(Session::has('couponinfo') &&
                !empty(Session::get('couponinfo')['coupon_code']))
                value="{{ Session::get('couponinfo')['coupon_code'] }}" @endif class="form-control form-control-sm"
                placeholder="Enter Coupon Code">

            <div class="input-group-append">
                <button class=" primaryBtn borderBtn" type="button" value="Apply" id="CouponBtn">APPLY</button>
            </div>
        ...
        <div class="couponCard cart-total-box ">
            ...
            <div class="">
                <div class="couponHeader">
                    <span class="couponTitle">Code: <strong class="couponCode coupon-key{{$key}}">{{
                            $available_coupon['coupon_code'] }}</strong></span>
                    @if(Session::has('couponinfo') && Session::get('couponinfo')['coupon_code'] ==
                    $available_coupon['coupon_code'])
                ...
                <ul class="couponDetails">
                    <li>Coupon Discount:<strong><span class="highlightDiscount"> {{ $available_coupon['amount']
                                }}@if($available_coupon['amount_type'] == 'Percentage')%@else Rs @endif Off
                            </span></strong>
                    </li>
                    <li>Coupon Validity:<strong> {{date('d-m-Y', strtotime($available_coupon['expiry_date']))}}</strong>
                    </li>
                </ul>
            </div>
        ...
        <div class="cart-total-box mt-2">
            ...
            <div class="subtotal-item shipping-box">
                ...
                <p class="subtotal-value"><strong>@if(!empty( $summery['shipping'])) ₹{{
                        amount_format($summery['shipping']) }} @else Free @endif</strong></p>
            </div>
...
<div class="col-12">
    <div class="text-align">
        ...
        <h5 class="mt-5">Your shopping cart is currently empty. <br>Start <a href="{{ route('home') }}">Shopping</a> to
            fill it up!</h5>
    </div>
</div>
@endif